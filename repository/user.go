...
func (p *userRepository) FindByID(ctx context.Context, id int) (*model.User, error) {
	...
	user := &model.User{}
	if err := p.Cfg.Database().WithContext(ctx).Find(user, id).Error; err != nil {
		return nil, err
	}
	return user, nil
...
func (p *userRepository) UpdateByID(ctx context.Context, id int, user *model.User) (*model.User, error) {
	...
	currentUser, err := p.FindByID(ctx, id)
	if err != nil {
		return nil, err
	}
	if err := p.Cfg.Database().WithContext(ctx).Model(currentUser).Updates(user).Error; err != nil {
		return nil, err
	}
